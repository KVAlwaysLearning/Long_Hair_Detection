...
def setup_environment():
    ...
    # This check is key: does config.json actually exist in that path?
    if not os.path.exists(os.path.join(parsing_path, "config.json")):
        st.error(f"Critical Error: Config file not found in {parsing_path}. Files found: {os.listdir(parsing_path)}")
        return None
