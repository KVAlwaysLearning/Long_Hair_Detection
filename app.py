import streamlit as st
import os
import gdown
import zipfile
import cv2
import torch
import torch.nn.functional as F
import numpy as np
import mediapipe as mp
from mediapipe.tasks import python
from mediapipe.tasks.python import vision
from transformers import pipeline, SegformerImageProcessor, SegformerForSemanticSegmentation
from PIL import Image
import matplotlib.pyplot as plt

# --- CONFIGURATION ---
DRIVE_FILE_ID = "1xLgUm3YgyvgzaL86LD_QFYPTRkHmU_Sd"
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODELS_ROOT = os.path.join(BASE_DIR, "models")

@st.cache_resource
def setup_environment():
    # Define where the folder should end up locally
    # We want it to be named 'Models' in your project root
    local_models_dir = os.path.join(BASE_DIR, "Models")

    # Download the whole folder if it doesn't exist
    if not os.path.exists(local_models_dir):
        # gdown can download an entire public folder using its ID
        gdown.download_folder(id='1xLgUm3YgyvgzaL86LD_QFYPTRkHmU_Sd', output=BASE_DIR, quiet=False)
        # Note: Depending on how gdown downloads, it might create 
        # a folder named 'Models' inside BASE_DIR automatically.
    
    # Paths now reference your specific subfolder structure
    model_files_dir = os.path.join(local_models_dir, "model_files")
    hf_root = os.path.join(local_models_dir, "hf_models")
    landmarker_path = os.path.join(model_files_dir, "face_landmarker.task")
    detector_path = os.path.join(model_files_dir, "detector.tflite")
    parsing_path = os.path.join(hf_root, "face-parsing")
    age_path = os.path.join(hf_root, "age-classifier")
    gender_path = os.path.join(hf_root, "gender-classifier") 
    
    # 2. Load Models
    proc = SegformerImageProcessor.from_pretrained(parsing_path)
    model = SegformerForSemanticSegmentation.from_pretrained(parsing_path)
    
    landmarker = vision.FaceLandmarker.create_from_options(
        vision.FaceLandmarkerOptions(base_options=python.BaseOptions(model_asset_path=landmarker_path), num_faces=1)
    )
    detector = vision.FaceDetector.create_from_options(
        vision.FaceDetectorOptions(base_options=python.BaseOptions(model_asset_path=detector_path))
    )
    
    age_pipe = pipeline("image-classification", model=age_path, top_k=None)
    gender_pipe = pipeline("image-classification", model=gender_path)
    
    return proc, model, landmarker, detector, age_pipe, gender_pipe

# --- LOGIC ---
proc, model, landmarker, detector, age_pipe, gender_pipe = setup_environment()
BUCKET_MAP = {'0-2': 1, '3-9': 6, '10-19': 15, '20-29': 25, '30-39': 35, '40-49': 45, '50-59': 55, '60-69': 65, '70+': 75}

def process_image(image_file):
    img = cv2.imdecode(np.frombuffer(image_file.read(), np.uint8), 1)
    h, w, _ = img.shape
    
    # Age/Gender (Crop)
    mp_img = mp.Image(image_format=mp.ImageFormat.SRGB, data=cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    det = detector.detect(mp_img).detections[0].bounding_box
    face_crop = img[int(det.origin_y):int(det.origin_y + det.height), int(det.origin_x):int(det.origin_x + det.width)]
    pil_face = Image.fromarray(cv2.cvtColor(face_crop, cv2.COLOR_BGR2RGB))
    
    age_res = age_pipe(pil_face)
    gender_res = gender_pipe(pil_face)[0]['label']
    exact_age = sum([item['score'] * BUCKET_MAP.get(item['label'], 0) for item in age_res if item['label'] in BUCKET_MAP])

    # Hair Logic
    land_res = landmarker.detect(mp_img)
    chin_y = int(land_res.face_landmarks[0][152].y * h) if land_res.face_landmarks else int(h * 0.65)
    
    inputs = proc(images=cv2.cvtColor(img, cv2.COLOR_BGR2RGB), return_tensors="pt")
    with torch.no_grad(): outputs = model(**inputs)
    
    parsing_map = F.interpolate(outputs.logits, size=(h, w), mode="bilinear", align_corners=False).argmax(dim=1).squeeze(0).cpu().numpy()
    hair_mask = cv2.morphologyEx((parsing_map == 13).astype(np.uint8), cv2.MORPH_OPEN, np.ones((5,5), np.uint8))
    lowest_hair_y = np.max(np.where(np.any(hair_mask > 0, axis=1))[0]) if np.any(hair_mask > 0) else 0
    
    hair_label = "LONG HAIR" if lowest_hair_y > (chin_y + 20) else "SHORT HAIR / BALD"
    final_gender = "male" if (20 <= exact_age <= 30 and "SHORT" in hair_label) else ("female" if (20 <= exact_age <= 30 and "LONG" in hair_label) else gender_res)

    return {"Age": round(exact_age, 1), "Hair": hair_label, "Gender": final_gender}, cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

# --- UI ---
st.title("Face Analysis Engine")
uploaded_file = st.file_uploader("Upload Image", type=["jpg", "png"])
if uploaded_file:
    res, img = process_image(uploaded_file)
    st.image(img)
    st.write(res)
