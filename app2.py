...
def setup_environment():
    ...
    # 2. PATH FINDER: Look deep for the specific subfolders
    ...
    # DEBUG: Help you see if it found them
    if not hf_root or not model_files_dir:
        st.error(f"Folders not found. Scanned: {MODELS_ROOT}")
        st.write(f"Actual structure found: {list(os.walk(MODELS_ROOT))}")
        return None
